...
sa_sed 's/contract Verifier/library Groth16Verifier/' $VERIFIER_FILE

# Use internal instead of public functions
sa_sed 's/public view/internal view/' $VERIFIER_FILE

# Read proof argument from memory instead of calldata
sa_sed 's/calldata proof/memory proof/' $VERIFIER_FILE
sa_sed $'s/calldatacopy(f, proof, 0x100)/mstore(f, mload(add(proof, 0x00)))\
...
sa_sed 's/calldata input/memory input/' $VERIFIER_FILE
sa_sed 's/calldataload(input)/mload(input)/' $VERIFIER_FILE
sa_sed 's/calldataload(add(input, 32))/mload(add(input, 32))/' $VERIFIER_FILE
sa_sed 's/calldataload(add(input, 64))/mload(add(input, 64))/' $VERIFIER_FILE

# Import verifier library with renamed filename
sa_sed 's/import {Verifier} from ".\/verifier.sol";/import {Groth16Verifier} from ".\/Groth16Verifier.sol";\
   import {isCDK} from "..\/utils\/Constants.sol";/' $VERIFIER_EXTENSIONS_FILE

# Use extensions as library instead of contract
sa_sed 's/contract Query is Verifier {/library Groth16VerifierExtensions {/' $VERIFIER_EXTENSIONS_FILE
sa_sed 's/CIRCUIT_DIGEST/Groth16Verifier.CIRCUIT_DIGEST/' $VERIFIER_EXTENSIONS_FILE
sa_sed 's/this.verifyProof/Groth16Verifier.verifyProof/' $VERIFIER_EXTENSIONS_FILE

# Use internal instead of public functions
sa_sed 's/public view/internal view/' $VERIFIER_EXTENSIONS_FILE

# Patch `verifyQuery` function to be `view` instead of `pure`
sa_sed '/function verifyQuery.*/,+2 s/pure/view/' $VERIFIER_EXTENSIONS_FILE

# Patch `verifyQuery` function to skip blockhash verification for polygon CDK chains
sa_sed 's/blockHash == query.blockHash/isCDK() || blockHash == query.blockHash/' $VERIFIER_EXTENSIONS_FILE
