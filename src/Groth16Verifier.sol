...
library Groth16Verifier {
    ...
    uint256 constant ALPHA_X =
        21878428251039175336140240668645524009296647141545715241416593840516886199851;
    uint256 constant ALPHA_Y =
        7613796246703746956474036927243978225259953255576333670362427739164077088196;

    // Groth16 beta point in G2 in powers of i
    uint256 constant BETA_NEG_X_0 =
        16791757900148876284476216265371474918597012898116639386553865504611770673199;
    uint256 constant BETA_NEG_X_1 =
        8046147793144561237074566172847568558796001307946094362674728200956291512892;
    uint256 constant BETA_NEG_Y_0 =
        3581131546766068770890158719834964135347996173968869291206866835210627193549;
    uint256 constant BETA_NEG_Y_1 =
        11371167896270637758211992380437897400756716153753513573435965591033258102260;

    // Groth16 gamma point in G2 in powers of i
    uint256 constant GAMMA_NEG_X_0 =
        15656251676249576926961928168143772075372145816788742924349928207424760401644;
    uint256 constant GAMMA_NEG_X_1 =
        19417410611222586413880371305851261317460616467758200725719730296142071083927;
    uint256 constant GAMMA_NEG_Y_0 =
        2304238467190076851298872717632010244634787145879580560883771667389749167169;
    uint256 constant GAMMA_NEG_Y_1 =
        10928155383073490334557008061618665107509010649975430820714288374129895998668;

    // Groth16 delta point in G2 in powers of i
    uint256 constant DELTA_NEG_X_0 =
        8486092294470017342177189857460329364057086155974154502523765691494360544681;
    uint256 constant DELTA_NEG_X_1 =
        15174422357781736479732901142735512674798735152609727203269451233269340357542;
    uint256 constant DELTA_NEG_Y_0 =
        8696101351045646455643688724904412840838705789785076166748247290113763255296;
    uint256 constant DELTA_NEG_Y_1 =
        12071245825334561150441714498905987107286196183226178431016979794045290559332;

    // Constant and public input points
    uint256 constant CONSTANT_X =
        13534064335417320411922877163336523243396839854045052763714713340224488695770;
    uint256 constant CONSTANT_Y =
        18410517048447217042897426418813646853841204871022017943281395617268525824783;
    uint256 constant PUB_0_X =
        1956467595872841022994555211460953441362256341316308114914183579577294266014;
    uint256 constant PUB_0_Y =
        19975759579112422379923539863786892923919577899435279612651118567795380269828;
    uint256 constant PUB_1_X =
        19522188431870224039169260173694908154290443650735863889093931529397710917391;
    uint256 constant PUB_1_Y =
        955811297761555870619532091879116098133047523457161566608838709312059248535;
    uint256 constant PUB_2_X =
        10145203286297443362334286840149616186330760248508871619133282996144765538862;
    uint256 constant PUB_2_Y =
        12966679014855430368613776817547897457980291726726234961021046369904301082392;
    ...
    function respond(uint8[] calldata data, Query calldata query)
        internal
        view
    ...
    function verifyGroth16Proof(uint8[] calldata data)
        internal
        view
        returns (uint256[3] memory inputs)
    {
    ...
    // Parse the plonky2 public inputs. and calculate the hash. Then asset this hash value must be
    function verifyQuery(uint8[] calldata data, Query calldata query)
        internal
        pure
    {
        ...
        address contract_address =
            convertToAddress(data, PI_CONTRACT_ADDR_OFFSET);
        require(
    ...
    function parseNftIds(uint8[] calldata data)
        internal
        pure
        returns (uint32[] memory)
    {
    ...
    function convertToU32(uint8[] calldata data, uint32 offset)
        internal
        pure
    ...
    function convertToU256(uint8[] calldata data, uint32 offset)
        internal
        pure
    ...
    function convertToAddress(uint8[] calldata data, uint32 offset)
        internal
        pure
        returns (address)
    {
    ...
    function convertToHash(uint8[] calldata data, uint32 offset)
        internal
        pure
        returns (uint256 result)
    {
